...
logger = logging.getLogger("gomining_daily_claim.browser_tools")
...
async def wait_for(
    ...
    single: bool = True,
    timeout_seconds: int = DEFAULT_SELECTOR_TIMEOUT,
):
    """
    ...
    if (selector is None and xpath is None) or (
        selector is not None and xpath is not None
    ):
    ...
    if last_error:
        logger.debug(
            f"Selector '{selector}' not found after {timeout_seconds}s: {last_error}"
        )
    return None
...
async def wait_for_page_load(
    ...
    wait_xpath: Optional[str] = None,
    wait_until: Optional[Callable[[zd.Tab], Coroutine[Any, Any, bool]]] = None,
    from_url: Optional[str] = None,
    timeout_seconds: int = DEFAULT_LOADING_TIMEOUT,
):
    """
    ...
    if wait_until is None and wait_selector is None and wait_xpath is None:
        raise ValueError(
            "At least one of wait_until, wait_selector, or wait_xpath must be provided."
        )
    if (wait_until is not None) and (
        wait_selector is not None or wait_xpath is not None
    ):
        raise ValueError(
            "wait_until cannot be used together with wait_selector or wait_xpath."
        )
    ...
    while from_url is not None and tab.url.startswith(from_url):
        if time.time() - start_time >= timeout_seconds:
            logger.error("Waiting for login redirect timed out.")
            return False
        await tab
        await asyncio.sleep(POLL_INTERVAL_SECONDS)
    ...
    if wait_until is not None:
        while time.time() - start_time < timeout_seconds:
            try:
                if await wait_until(tab):
                    logger.info("Page loaded based on custom condition.")
                    return True
            except Exception as exc:
                logger.debug(f"Error while waiting for custom condition: {exc}")
            await asyncio.sleep(POLL_INTERVAL_SECONDS)
        logger.error("Waiting for page load based on custom condition timed out.")
        return False
    else:
        ...
        nav = await wait_for(
            ...
            xpath=wait_xpath,
            timeout_seconds=max(10, remaining_time),
        )
        if nav is None:
            logger.error("Navigator element not found after timeout.")
            return False
        logger.info("Navigator element found. Page loaded.")
        return True
...
async def safe_save_screenshot(tab: zd.Tab, filename: str):
    ...
    try:
        ...
        logger.warning(f"Failed to save screenshot {filename}: {exc}")
